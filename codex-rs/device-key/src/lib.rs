...
const DEVICE_KEY_ID_PREFIX: &str = "dk_";
const DEVICE_KEY_ID_RANDOM_BYTES: usize = 32;
const DEVICE_KEY_ID_ENCODED_BYTES: usize = 43;
const DEVICE_KEY_ID_LEN: usize = DEVICE_KEY_ID_PREFIX.len() + DEVICE_KEY_ID_ENCODED_BYTES;
const INVALID_DEVICE_KEY_ID_MESSAGE: &str =
    "keyId must be dk_ followed by unpadded base64url-encoded 32 bytes";
...
/// Keeps all externally supplied key IDs inside the random `dk_` namespace created by this crate.
///
...
fn validate_key_id(key_id: &str) -> Result<(), DeviceKeyError> {
    let Some(encoded_key) = key_id.strip_prefix(DEVICE_KEY_ID_PREFIX) else {
        return Err(DeviceKeyError::InvalidPayload(
...
mod tests {
    ...
    fn assert_valid_generated_key_id(key_id: &str) {
        ...
        let encoded_key = key_id
            ...
            .expect("generated key id should use dk_ prefix");
    ...
    fn create_generates_distinct_key_ids() {
        ...
        assert_valid_generated_key_id(&first.key_id);
        assert_valid_generated_key_id(&second.key_id);
    }
    ...
    fn key_id_validation_rejects_untrusted_namespaces() {
        ...
        for key_id in [
            ...
            format!("dk_{}", &valid_suffix[..DEVICE_KEY_ID_ENCODED_BYTES - 1]),
            format!("dk_{valid_suffix}A"),
            format!("dk_{}=", &valid_suffix[..DEVICE_KEY_ID_ENCODED_BYTES - 1]),
            format!("dk_{}+", &valid_suffix[..DEVICE_KEY_ID_ENCODED_BYTES - 1]),
        ] {
